...
try {
	...
	logger.warn(
		"sharp library not found. Image compression will be limited. Install with: npm install sharp",
	);
}
...
export class ImageProcessor {
	...
	static async compressImage(
		...
		try {
			...
			if (!sharp) {
				logger.warn(
					`Image size (${(originalSize / 1024 / 1024).toFixed(2)}MB) exceeds API limit. Sharp library not installed.`,
				);
				if (originalSize > target * 2) {
					throw new Error(
						`Image is too large (${(originalSize / 1024 / 1024).toFixed(2)}MB). Please use a smaller image or install sharp: npm install sharp`,
					);
				}
				return {
					compressed: false,
			...
			logger.info(
				`Compressing image: ${(originalSize / 1024 / 1024).toFixed(2)}MB → target: ${(target / 1024 / 1024).toFixed(2)}MB`,
			);
			...
			while (attempts < maxAttempts) {
				...
				if (
					...
					logger.info(
						`Resizing image: ${metadata.width}x${metadata.height} → ${newWidth}x${newHeight}`,
					);
				...
				if (compressedBuffer.length <= target) {
					logger.info(
						`✅ Compression successful: ${(originalSize / 1024 / 1024).toFixed(2)}MB → ${(compressedBuffer.length / 1024 / 1024).toFixed(2)}MB (quality: ${quality})`,
					);
					return {
						compressed: true,
						originalSize,
				...
				if (quality < 30) {
					...
					logger.warn(
						`Reached minimum quality threshold. Final size: ${(compressedBuffer.length / 1024 / 1024).toFixed(2)}MB`,
					);
					break;
				}
			}
			...
			if (compressedBuffer.length > target * 1.5) {
				...
				throw new Error(
					`Unable to compress image to target size. Final size: ${(compressedBuffer.length / 1024 / 1024).toFixed(2)}MB (target: ${(target / 1024 / 1024).toFixed(2)}MB)`,
				);
			}

			logger.warn(
				`⚠️  Compression reached ${(compressedBuffer.length / 1024 / 1024).toFixed(2)}MB (target: ${(target / 1024 / 1024).toFixed(2)}MB)`,
			);
			return {
				compressed: true,
				originalSize,
