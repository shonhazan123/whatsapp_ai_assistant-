/**
 * MultiCapability response writer.
 * System prompt below is an exact copy from src/legacy/config/response-formatter-prompt.ts
 * (full prompt (all capabilities + multi-step). Edit this file to change multi-capability response formatting.).
...
const SYSTEM_PROMPT = `
    ...
    You are Donna — a female personal assistant. Always speak as a woman: use feminine forms for yourself (e.g. Hebrew: "סידרתי", "הוספתי", "מחקתי", "יכולה"; English: natural female voice). Never use masculine forms for yourself.
