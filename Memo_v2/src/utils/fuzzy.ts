 ...
 * 
 * Adapted from V1 src/utils/fuzzy.ts
...
/**
 * Convert similarity threshold (0-1, higher=better) to Fuse distance (0-1, lower=better)
 */
function toFuseThreshold(similarity: number): number {
  return 1 - similarity;
}
...
export class FuzzyMatcher {
  ...
  static search<T>(
    ...
    const fuse = new Fuse(items, {
      keys,
      threshold: toFuseThreshold(threshold),
      includeScore: true,
      includeMatches: true,
      ignoreLocation: true,
   ...
   */
  static isSimilar(str1: string, str2: string, threshold: number = DEFAULT_THRESHOLD): boolean {
