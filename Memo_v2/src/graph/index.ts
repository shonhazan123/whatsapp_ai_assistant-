...
function plannerRouter(state: MemoState): string {
  ...
  if (!plannerOutput) {
    console.log(`[plannerRouter] No planner output, routing to capability_check`);
  ...
  console.log(`[plannerRouter] Intent: ${plannerOutput.intentType}, routing to capability_check`);
...
function capabilityCheckRouter(state: MemoState): string {
  ...
  if (state.finalResponse) {
    console.log(`[capabilityCheckRouter] Capabilities missing, routing to response_writer`);
    return 'response_writer';
  }

  // All capabilities available, continue to HITL gate
  console.log(`[capabilityCheckRouter] All capabilities available, routing to hitl_gate`);
  return 'hitl_gate';
}
...
export async function hasPendingInterrupt(threadId: string): Promise<boolean> {
  ...
  try {
    ...
    // Handle timeout case
    if (state === null) {
      console.warn(`[MemoGraph] getState timed out for ${threadId} after 5000ms, treating as no pending interrupt`);
      return false;
    }

    const duration = Date.now() - startTime;
    console.log(`[MemoGraph] State retrieved in ${duration}ms, next: ${state?.next?.length || 0} pending`);

    return state?.next?.length > 0;
  } catch (error) {
    const duration = Date.now() - startTime;
    console.error(`[MemoGraph] hasPendingInterrupt error after ${duration}ms:`, error);
    return false;
  }
}
...
export async function invokeMemoGraph(
  ...
  if (isPendingInterrupt) {
    ...
    if (state === null) {
      console.warn(`[MemoGraph] getState timed out when checking interrupt, treating as fresh invocation`);
      isPendingInterrupt = false;
    } else {
      // Check for HITL timeout using interruptedAt from payload metadata
      const interruptPayload = state?.tasks?.[0]?.interrupts?.[0]?.value as InterruptPayload | undefined;
      const interruptedAt = interruptPayload?.metadata?.interruptedAt as number | undefined;

      if (interruptedAt && Date.now() - interruptedAt > INTERRUPT_TIMEOUT_MS) {
        console.log(`[MemoGraph] Interrupt timed out after ${INTERRUPT_TIMEOUT_MS}ms, cleaning up stale thread`);
