...
export class ResponseFormatterNode extends CodeNode {
  ...
  protected async process(state: MemoState): Promise<Partial<MemoState>> {
    ...
    for (const [stepId, result] of executionResults) {
      if (result.success && result.data) {
        ...
        if (step) {
          ...
          const stepContext = this.buildResponseContext([result.data], step.capability, step.action);
    ...
    console.log(`[ResponseFormatter] Built response for ${capability}:${action}` +
      (isMultiCapability ? ` (multi-capability: ${Array.from(uniqueCapabilities).join(', ')})` : '') +
  ...
  private extractDatabaseContext(
    ...
    const items = this.extractItemsArray(data, 'database') as DatabaseTaskResult[];
    ...
    console.log(`[ResponseFormatter] Database context: isReminder=${context.isReminder}, hasDueDate=${context.hasDueDate}, isToday=${context.isToday}, itemCount=${items.length}`);
    return context;
  }
  ...
  private extractCalendarContext(
    ...
    // Extract items array from various wrapper structures
    const events = this.extractItemsArray(data, 'calendar') as CalendarEventResult[];
    const meta = this.extractMetadata(data);
    ...
    for (const item of events) {
      ...
      if (item.recurrence || (Array.isArray(item.days) && item.days.length > 0) || item.recurringEventId) {
        context.isRecurring = true;
      }
    ...
    console.log(`[ResponseFormatter] Calendar context: isRecurring=${context.isRecurring}, isRecurringSeries=${context.isRecurringSeries}, eventCount=${events.length}`);
    return context;
  }
  ...
  private extractGmailContext(
    ...
    console.log(`[ResponseFormatter] Gmail context: isPreview=${context.isPreview}, isSent=${context.isSent}, emailCount=${emails.length}`);
    return context;
  }
  ...
  private extractSecondBrainContext(
    ...
    // Extract items array from various wrapper structures
    const memories = this.extractItemsArray(data, 'second-brain') as SecondBrainResult[];
    const meta = this.extractMetadata(data);
    ...
    console.log(`[ResponseFormatter] SecondBrain context: isStored=${context.isStored}, isSearch=${context.isSearch}, memoryCount=${memories.length}`);
    return context;
  }
