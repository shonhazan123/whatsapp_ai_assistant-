...
export class ResponseFormatterNode extends CodeNode {
	...
	protected async process(state: MemoState): Promise<Partial<MemoState>> {
		...
		// Pass through so ResponseWriter can extract data.response without another LLM call.
		if (capability === "meta") {
			console.log(
				"[ResponseFormatter] Packaging meta response (already LLM-generated by MetaResolver)",
			);
