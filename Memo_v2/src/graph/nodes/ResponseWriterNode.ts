...
export class ResponseWriterNode extends CodeNode {
  ...
  protected async process(state: MemoState): Promise<Partial<MemoState>> {
    ...
    // Meta responses: MetaResolver already produced the final message via its LLM call.
    // Pass through without another LLM call.
    if (formattedResponse.agent === 'meta') {
      console.log('[ResponseWriter] Using meta response directly (already LLM-generated by MetaResolver)');
      const metaData = Array.isArray(formattedResponse.formattedData)
      ...
      console.warn('[ResponseWriter] Meta response data missing response field');
      const fallbackMessage = language === 'he'
        ? 'לא הצלחתי לעבד את הבקשה. נסה שוב בבקשה.'
        : "I couldn't process your request. Please try again.";
      return { finalResponse: fallbackMessage };
    }
